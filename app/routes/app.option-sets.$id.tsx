...
export const action = async ({ request, params }: ActionFunctionArgs) => {
    ...
    try {
        ...
        console.log("Option Set Save Action hit. Data:", dataStr);
...
export default function EditOptionSetPage() {
    ...
    const { optionSet, products } = useLoaderData<typeof loader>();
    ...
    const [groups, setGroups] = useState<any[]>(() => {
        try {
            return JSON.parse(optionSet.groups || "[]");
        } catch (e) {
            return [{ id: 'group-1', name: 'Group', expanded: true, elements: [] }];
        }
    });

    const [activeGroupId, setActiveGroupId] = useState<string>(() => {
        const parsedGroups = JSON.parse(optionSet.groups || "[]");
        return parsedGroups.length > 0 ? parsedGroups[0].id : 'group-1';
    });

    const [editingGroupId, setEditingGroupId] = useState<string | null>(null);
    const [editingGroupName, setEditingGroupName] = useState("");

    const [optionSetName, setOptionSetName] = useState(optionSet.title || "Untitled Option Set");
    const [optionSetStatus, setOptionSetStatus] = useState(optionSet.status || "active");
    ...
    useEffect(() => {
        ...
        console.log("--- SUCCESS WATCHER check ---", {
            state: fetcher.state,
            wasSubmitting,
            isNowIdle,
            isSuccess,
        ...
        if (hasSubmitted.current && wasSubmitting && isNowIdle && isSuccess) {
            console.log("Save successful! Closing builder...");
    ...
    const [selectionMode, setSelectionMode] = useState<'manual' | 'automatic' | 'all'>(optionSet.selectionMode as any || 'all');
    const [selectedProducts, setSelectedProducts] = useState<any[]>(() => {
        try {
            return JSON.parse(optionSet.selectedProducts || "[]");
        } catch (e) {
            return [];
        }
    });
    const [conditions, setConditions] = useState(() => {
        try {
            return JSON.parse(optionSet.conditions || "[]");
        } catch (e) {
            return [{ id: Date.now(), type: 'tag', operator: 'equals', value: '' }];
        }
    });
    ...
    const handleOpenEditModal = (group: any) => {
        ...
        setEditingGroupName(group.name || group.title || "Group");
        setTimeout(() => {
    ...
    const handleSaveGroupName = () => {
        setGroups(groups.map(g => {
            if (g.id === editingGroupId) {
                return { ...g, name: editingGroupName, title: editingGroupName };
            }
            return g;
        }));
        setTimeout(() => {
    ...
    return (
        ...
        >
            ...
            {/* Hidden form OUTSIDE all Shopify web components so Shadow DOM can't intercept */}
            <fetcher.Form
                ref={formRef}
                method="post"
                style={{ display: 'none' }}
            >
                <input type="hidden" name="data" value={savePayload} />
            </fetcher.Form>
                                ...
                                <s-grid gridTemplateColumns="1fr auto" alignItems="center">
                                    ...
                                    <s-stack direction="inline" gap="small" alignItems="center">
                                        ...
                                        <button
                                            ...
                                            disabled={isSaving}
                                            onClick={(e) => {
                                                console.log("--- SAVE BUTTON HANDLER START ---");
                                                e.stopPropagation();
